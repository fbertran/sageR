...
gps_coords <- mapply(geo, city= Precipitations_USA[,1], state = Precipitations_USA[,4], method = "osm")
...
```{r, eval=LOCAL}
precip_gps_map <- get_map(c(left = min(gps_coords_df$long)*1.03, bottom = min(gps_coords_df$lat)/1.2, right = max(gps_coords_df$long)/1.12, top = max(gps_coords_df$lat))*1.03, source = "osm", zoom=6, messaging = FALSE)
```


```{r, eval=LOCAL}
ggmap(precip_gps_map,extent = "device") + 
...
ggsave("map_pprecip_USA.pdf", width=14, height=8)
```


```{r, eval=LOCAL}
precip_gps_map_bw <- get_stamenmap(c(left = min(gps_coords_df$long)*1.03, bottom = min(gps_coords_df$lat)/1.2, right = max(gps_coords_df$long)/1.12, top = max(gps_coords_df$lat))*1.03, source = "stamen", zoom=6, maptype="toner")
```


```{r, eval=LOCAL}
ggmap(precip_gps_map_bw,extent = "device") + 
  geom_point(aes(x = long, y = lat, color=`Precipitation (cms)`),
...
```{r, eval=SAVE_GRAPHS, echo=SAVE_GRAPHS}
ggsave("map_pprecip_USA_bw.pdf", width=14, height=8)
```


```{r, eval=LOCAL}
data(Resistance)
Resistance<-c(219.92,216.71,219.90,218.61,216.06,214.85,219.23,214.00,223.43,221.64,
  223.39,218.57,215.00,221.90,222.23,215.42,218.85,222.22,219.45,216.99,
  220.94,217.77,225.23,217.66,215.56,219.46,215.75,215.78,216.91,226.63)
sigma.test(Resistance,sigmasq = 8)
```
